...
const USER_PUBLIC_SELECT = 'id, email, name, role, plan, plan_expires_at'
...
router.get('/me', authMiddleware, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT ${USER_PUBLIC_SELECT} FROM users WHERE id = $1`,
      [req.userId]
    )
...
router.put('/profile', authMiddleware, authLimiter, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `UPDATE users SET name = $1 WHERE id = $2 RETURNING ${USER_PUBLIC_SELECT}`,
